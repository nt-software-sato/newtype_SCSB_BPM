...
public partial class A61CallFERP : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        string GRMAINNO = HttpUtility.HtmlEncode(Request.QueryString["GRMAINNO"]) ?? string.Empty;
        ...
        */
        /*
        Type = "1";
        GRMAINNO = "20190812-005";
        ISCONVERTPREPAYMENT = "N";
        ALLOCATIONSTARTYEARMONTH = "201906";
        ALLOCATIONENDYEARMONTH = "201910";
        RequisitionID = "test";
        WITHHOLDINGINCOMETAXRATIO = 0.1;
        WITHHOLDINGINCOMEACCOUNTID = "214500100";
        WITHHOLDINGINHEALTHINSURANCETAXRATIO = 0.1;
        WITHHOLDINGINHEALTHINSURANCEPREMIUMACCOUNTID = "214500400";
        */
        ...
        if (Type == "2")
        {
            var Result = new
            {
                ...
                USECASE = USECASE,
                SERVICEID = "EXEPURCHASEPAYMENTENTRYBYBCREATE"
            };
            //序列化為JSON字串並輸出結果
        ...
        else
        {
            var Result = new
            {
                ...
                WITHHOLDINGINHEALTHINSURANCEPREMIUMACCOUNTID = WITHHOLDINGINHEALTHINSURANCEPREMIUMACCOUNTID,
                USECASE = USECASE,
                SERVICEID = "EXEPURCHASEPAYMENTENTRYBYBACQUIRE"
            };
            //序列化為JSON字串並輸出結果
            JsonText = JsonConvert.SerializeObject(Result);
            //Response.Write(JsonText);
        ...
        if (Type=="1")
        {
            ...
            using (SqlConnection conn = new SqlConnection(connString))
            {
                ...
                try
                {
                    TmpSql = "Delete from FM7T_A61_EXEPURCHASEPAYMENTENTRYBYAACQUIRE WHERE RequisitionID = @RequisitionID ";
                    ...
                    using (SqlBulkCopy sqlBC = new SqlBulkCopy(conn, SqlBulkCopyOptions.KeepNulls, tran))
                    {
                        ...
                        sqlBC.DestinationTableName = "FM7T_A61_EXEPURCHASEPAYMENTENTRYBYAACQUIRE";
                        ...
                        sqlBC.ColumnMappings.Add("VALUEDATE", "VALUEDATE");
                        sqlBC.ColumnMappings.Add("WRITINGOFFSEQUENCENO", "WRITINGOFFSEQUENCENO");
                        sqlBC.ColumnMappings.Add("ENTRYCURRENCYID", "ENTRYCURRENCYID");

                        sqlBC.WriteToServer(DetailDt);
                        tran.Commit();
                    }
                    Response.Write(OutputJosn(1, "一般請款分錄取得成功", ""));
                }
                catch (Exception ex)
                {
                    tran.Rollback();
                    Response.Write(OutputJosn(99, "一般請款分錄寫入發生錯誤", ""));
                }
            }
        }
        else
